...
export const axiosInstance = async (URL, METHOD, DATA = {}, MESSAGE, token,) => {
  ...
  try {
    ...
    if (response?.data?.message != "Get all fresh blogs" 
 
        ) {
      toast.success(response?.data?.message || MESSAGE || response?.massage);
    }
...
const MyComponent = () => {
  ...
  const handleApiCall = async () => {
    const result = await axiosInstance('/your-endpoint', 'GET', {}, 'Fetching data', token);
    console.log(result);
  };
